...
massqc_attach <- function() {
  ...
  #     right = paste0("massqc ", package_version("massqc"))
  #   ),
  #   startup = TRUE
  # )
...
msg <- function(..., startup = FALSE) {
  if (startup) {
    if (!isTRUE(getOption("massqc.quiet"))) {
...
massqc_packages <- function(include_self = TRUE) {
  raw <- utils::packageDescription("massqc")$Imports
  imports <- strsplit(raw, ",")[[1]]
  parsed <- gsub("^\\s+|\\s+$", "", imports)
  names <- vapply(strsplit(parsed, "\\s+"), "[[", 1, FUN.VALUE = character(1))
  
  if (include_self) {
    names <- c(names, "massqc")
  }
